...
public class Deconvolution {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int iterations = 15;
        Thread redRestoreThread = new RichardsonLucy(red, psf, psfFlipped, iterations);
        Thread greenRestoreThread = new RichardsonLucy(green, psf, psfFlipped, iterations);
        Thread blueRestoreThread = new RichardsonLucy(blue, psf, psfFlipped, iterations);

        redRestoreThread.start();
        greenRestoreThread.start();
        blueRestoreThread.start();
        redRestoreThread.join();
        greenRestoreThread.join();
        blueRestoreThread.join();

        float[][] redRestored = ((RichardsonLucy) redRestoreThread).getResultado();
        float[][] greenRestored = ((RichardsonLucy) greenRestoreThread).getResultado();
        float[][] blueRestored = ((RichardsonLucy) blueRestoreThread).getResultado();
